        ...
        if filename:
            ...
            if oldTask:
                ...
                if control:
                    ...
                    for task in taskInInterval:
                        if task[6] == "False":
                            dataInizio = datetime.datetime.fromtimestamp(float(task[4]) / 100).strftime(
                                '%d-%m-%Y %H:%M:%S')
                            dataFine = datetime.datetime.fromtimestamp(float(task[5]) / 100).strftime(
                                '%d-%m-%Y %H:%M:%S')
                    ...
                    for prog in progetti:
                        print(progdict[prog])
                        storeData(filename, ["", "", "Totale del progetto ", progdict[prog][0],
                                             intervalToStringOraMinSec(progetti[prog])])
